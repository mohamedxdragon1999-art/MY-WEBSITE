...
const env = { DB, ENCRYPTION_KEY: 'k'.repeat(32) };
...
{
  await call('PATCH', '/ai/settings', { provider: 'nvidia', nvidia_key: 'nv-key', openai_key: 'sk-key', model: 'nvidia/llama-3.1-nemotron-70b-instruct' }, A.token);
...
if (!NO_NET) {
  ...
  check('real catalog is substantial (>=25 chat models)', list.length >= 25, 'got ' + list.length);
