...
console.log('\n== 8. LIVE CANVAS (real DOM surface, not just a controller) ==');
{
  ...
  const sb = canvas.setBreakpoint('mobile');
  check('switching viewport ACTUALLY switches the controller breakpoint (was shadowed by a duplicate method name)', sb === canvas && canvas.breakpoint === 'mobile', String(canvas.breakpoint));
  check('switching viewport re-renders the live surface', container.innerHTML.length > 0 && container.innerHTML.includes('data-nx-id'));
  const nb = canvas.setNodeBreakpoint(hero, 'mobile', { display: 'block' });
  check('per-node responsive override goes through the canvas action (responsive.update op)', nb.ok && nb.ops.some(o => o.op === 'responsive.update'), JSON.stringify(nb).slice(0, 120));
