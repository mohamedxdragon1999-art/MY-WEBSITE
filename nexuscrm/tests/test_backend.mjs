...
{
  const s = await call('PATCH', '/ai/settings', { provider: 'nvidia', nvidia_key: 'nvapi-test-key-123456', openai_key: 'sk-test-key-123456' }, token);
  ...
  await call('PATCH', '/ai/settings', { openai_key: 'sk-test-key-123456' }, token);
...
{
  await call('PATCH', '/ai/settings', { provider: 'nvidia', nvidia_key: 'nvapi-test-key-123456', openai_key: 'sk-test-key-123456' }, token);
  ...
  await call('PATCH', '/ai/settings', { openai_key: 'sk-test-key-123456' }, token); // restore both for circuit test
...
{
  await call('PATCH', '/ai/settings', { nvidia_key: 'nvapi-test-key-123456', openai_key: 'sk-test-key-123456' }, token);
...
{
  await call('PATCH', '/ai/settings', { provider: 'nvidia', nvidia_key: 'nvapi-test-key-123456', model: 'bad/model-name,meta/llama-3.1-8b-instruct' }, token);
...
console.log('\n== V9: HEALTH TEST PER-PROVIDER MODEL + KIND ==');
{
  ...
  await call('PATCH', '/ai/settings', { provider: 'openai', model: 'gpt-4o-mini', nvidia_key: 'nvapi-test-key-123456', openai_key: '' }, token);
  let pingedModels = [];
  globalThis.fetch = async (url, opts = {}) => {
    const u = String(url);
    if (u.includes('nvidia.com')) {
  ...
  globalThis.fetch = FAKE_FETCH;
  aiBehavior = 'ok';
  await call('PATCH', '/ai/settings', { provider: 'nvidia', model: 'meta/llama-3.1-8b-instruct', nvidia_key: 'nvapi-test-key-123456' }, token);
}


console.log('\n== V9.2: HEALTH TEST — UNDECRYPTABLE STORED KEY MUST NOT CRASH /ai/health ==');
{
  // STEP 0 regression: simulate a key saved while ENCRYPTION_KEY existed,
  // then the secret going missing/rotated on the backend. The stored value
  // is enc:v1:... garbage that decryptSecret() throws on. Before the fix,
  // that throw escaped pingProvider() and crashed the WHOLE /ai/health
  // route (opaque 500 → "Test failed"), which the user experienced as
  // "the key is not being read". After: 200 + per-provider statuses.
  const u = await DB.prepare("SELECT workspace_id AS ws FROM users WHERE email='test@example.com'").first();
  await DB.prepare("UPDATE workspaces SET ai_nvidia_key=? WHERE id=?").bind('enc:v1:AAAA:BBBB', u.ws).run();
  if (globalThis.__nxTest) globalThis.__nxTest.resetProviderHealth();
  // Any outbound provider fetch here proves the crash path — but with the
  // fix, decrypt fails BEFORE any network call, so a fetch that throws on
  // NVIDIA URLs would only be reached if the bug came back.
  const h = await call('GET', '/ai/health', null, token);
  check('health route returns 200 even with an undecryptable key', h.status === 200, 'status=' + h.status);
  check('nvidia reports an encryption error (not a crash, not bad_key)', h.data?.nvidia?.status === 'error' && h.data?.nvidia?.kind === 'encryption', JSON.stringify(h.data?.nvidia));
  check('other providers unaffected (openai still reports no_key)', h.data?.openai?.status === 'no_key', JSON.stringify(h.data?.openai));
  check('encryption message tells the operator exactly what to do', /ENCRYPTION_KEY/i.test(h.data?.nvidia?.message || ''), (h.data?.nvidia?.message || '').slice(0, 90));
  // Restore a usable plaintext key for the sections that follow. IMPORTANT:
  // restore through the API (not raw SQL) — handleAISettings invalidates the
  // workspace cache; a raw UPDATE would leave the cached row holding the
  // enc:v1 garbage and poison every later AI call in this suite.
  await call('PATCH', '/ai/settings', { nvidia_key: 'nvapi-test-key-123456' }, token);
  if (globalThis.__nxTest) globalThis.__nxTest.resetProviderHealth();
}
...
console.log('\n== CRON SAFETY ==');
{
  // scheduled() defers ALL work through ctx.waitUntil and returns instantly —
  // a test must capture that promise and await it, or it races the sweep.
  let sweep;
  const cronCtx = { waitUntil: (p) => { sweep = p; } };
...
console.log('\n== V9.3: /ai/health refresh — manual Test never shows a stale verdict ==');
{
  // Cached path: same (failing) answer twice in a row is fine.
  aiBehavior = 'fail401';
  const h1 = await call('GET', '/ai/health', null, token);
  aiBehavior = 'ok';
  const h2 = await call('GET', '/ai/health', null, token);
  check('health result cached 30s (no refresh → stale answer)', h1.data?.nvidia?.status === h2.data?.nvidia?.status);
  // A manual Test (?refresh=1) MUST bypass the cache and see the new truth.
  const h3 = await call('GET', '/ai/health?refresh=1', null, token);
  check('?refresh=1 bypasses the health cache (verdict reflects the fix)', h3.data?.nvidia?.status !== h2.data?.nvidia?.status, h2.data?.nvidia?.status + ' → ' + h3.data?.nvidia?.status);
  aiBehavior = 'ok';
}


console.log('\n== STEP 1: /auth/logout — session destroyed, token REJECTED after logout ==');
{
  // Dedicated user so nothing else in the suite is affected.
  const reg = await call('POST', '/auth/register', { name: 'Logout Tester', email: 'logout-test@example.com', password: 'password123' });
  const tok = reg.data?.token;
  check('logout test user registered', reg.status === 200 && !!tok);

  const before = await call('GET', '/contacts', null, tok);
  check('session works before logout', before.status === 200);

  const lo = await call('POST', '/auth/logout', null, tok);
  check('logout returns ok', lo.status === 200 && lo.data?.ok === true);

  const after = await call('GET', '/contacts', null, tok);
  check('SAME token is rejected after logout (401)', after.status === 401, 'status=' + after.status);
  const me = await call('GET', '/auth/me', null, tok);
  check('/auth/me also rejects the dead token', me.status === 401);
  const lo2 = await call('POST', '/auth/logout', null, tok);
  check('repeated logout with the dead token → 401 (not ok)', lo2.status === 401);

  const wrongMethod = await call('GET', '/auth/logout', null, tok);
  check('GET /auth/logout is not a valid route (method enforced)', wrongMethod.status !== 200, 'status=' + wrongMethod.status);

  // Token specificity: logging out ONE session must not kill other sessions.
  const reg2 = await call('POST', '/auth/login', { email: 'test@example.com', password: 'password123' });
  const mainTok = reg2.data?.token;
  check('main user re-login ok', reg2.status === 200 && !!mainTok);
  await call('POST', '/auth/logout', null, mainTok);
  const still = await call('GET', '/contacts', null, token); // original suite token
  check('other sessions survive one logout (token-specific destroy)', still.status === 200, 'status=' + still.status);
}


console.log('\n== STEP 1: /auth/demo — isolated, rate-limited, purged by cron ==');
{
  // Isolation marker in the MAIN workspace.
  const probe = await call('POST', '/contacts', { name: 'MAIN-WS-SECRET-MARKER', email: 'marker@main.ws' }, token);
  check('marker contact created in main workspace', probe.status === 200 && probe.data?.id);

  const demo = await call('POST', '/auth/demo', null, null);
  check('demo login returns a token + @nexuscrm.local user', demo.status === 200 && !!demo.data?.token && /@nexuscrm\.local$/.test(demo.data?.user?.email || ''), JSON.stringify(demo.data?.user || {}).slice(0, 80));
  const demoTok = demo.data?.token;
  const demoEmail = demo.data?.user?.email;

  const me = await call('GET', '/auth/me', null, demoTok);
  check('demo session works for /auth/me', me.status === 200 && me.data?.user?.email === demoEmail);

  const demoContacts = await call('GET', '/contacts', null, demoTok);
  check('demo workspace starts EMPTY (cannot see main workspace data)', demoContacts.status === 200 && demoContacts.data?.total === 0, 'total=' + demoContacts.data?.total);

  const crossRead = await call('GET', `/contacts/${probe.data.id}`, null, demoTok);
  check('demo CANNOT read a main-workspace contact by id (cross-tenant 404)', crossRead.status === 404, 'status=' + crossRead.status);

  const demoWrite = await call('POST', '/contacts', { name: 'Demo Own Contact' }, demoTok);
  check('demo can create its own contact', demoWrite.status === 200);
  const mainTotal = await call('GET', '/contacts', null, token);
  check('main workspace untouched by demo write', (mainTotal.data?.total || 0) >= 1 && !JSON.stringify(mainTotal.data?.contacts || []).includes('Demo Own Contact'));

  // Rate limit: 10 demo workspaces per IP per hour. One already used above.
  let hit429 = false;
  for (let i = 0; i < 12 && !hit429; i++) {
    const r = await call('POST', '/auth/demo', null, null);
    if (r.status === 429) hit429 = true;
  }
  check('demo creation rate limit kicks in (429)', hit429);

  // Cron purge: age the demo user past the 2-day window, run the cron sweep,
  // and confirm the demo workspace is fully deleted while main survives.
  DB._raw(`UPDATE users SET created_at='2020-01-01T00:00:00.000Z' WHERE email='${demoEmail}'`);
  let sweep;
  const cronCtx = { waitUntil: (p) => { sweep = p; } };
  await worker.scheduled({}, env, cronCtx);
  await Promise.resolve(sweep).catch(() => {});
  const demoUserLeft = DB._raw(`SELECT COUNT(*) c FROM users WHERE email='${demoEmail}'`)[0].values[0][0];
  check('stale demo workspace purged by cron (user row gone)', demoUserLeft === 0);
  const demoWsLeft = DB._raw(`SELECT COUNT(*) c FROM workspaces WHERE id=${demo.data.user.workspace_id}`)[0].values[0][0];
  check('demo workspace row deleted too', demoWsLeft === 0);
  const mainUserLeft = DB._raw("SELECT COUNT(*) c FROM users WHERE email='test@example.com'")[0].values[0][0];
  check('main workspace survived the demo purge', mainUserLeft === 1);
  const deadDemoTok = await call('GET', '/auth/me', null, demoTok);
  check('purged demo session no longer authenticates', deadDemoTok.status === 401);
}
