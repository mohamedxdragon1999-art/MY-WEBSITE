...
## 🚀 Deploy from INSIDE the app — Settings → System → "Deploy my backend now"

The app deploys its own backend through the Deploy Studio — no console
commands, no copy-paste, no leaving the app (needs the app running via
`Start-NexusCRM`, which is how it is normally launched):

- **Option A — one click (recommended):** streams the battle-tested
  auto-deployer's live progress right into the Settings modal. The only thing
  you do is click **Allow** once in the Cloudflare login window.
- **Option B — API token (advanced):** paste a Cloudflare API token
  (Account Settings: Read · Workers Scripts: Edit · D1: Edit) and the app
  performs the full REST deploy itself: verifies the token, finds your
  account automatically, reuses-or-creates the D1 database, applies the whole
  schema in one batched call, uploads the worker module with its DB binding
  and `ENCRYPTION_KEY` secret, enables the `*.workers.dev` URL, schedules the
  automation cron, then health-checks the live backend and connects to it.
  The token is used once in memory — never stored, never logged.

Why the middle-man? Cloudflare's API refuses browser CORS, so the app talks
to it through the bundled local server's same-origin `/api/cf` proxy (that
proxy is allowlisted to deploy endpoints only). The same machinery
auto-deploys the **CORS proxy worker** from Settings → AI Providers →
"Fix Connection" too.

Total time: ~15 minutes, once — or **~2 minutes with one-click** (next section).

## ⚡ One-click auto-deploy (recommended — you don't do any of this manually)

Just double-click **`Start-NexusCRM`** (`.bat` on Windows, `.command` on
Mac). Before the app opens, the launcher checks itself:

- **Already deployed?** It verifies your backend is alive with a live health
  check and continues in ~2 seconds. It never redeploys what's already
  working — no duplicate databases, no wasted deploys.
- **Not deployed yet?** It offers to do everything for you (answer `y`):
  opens your browser to log into a **free Cloudflare account** (or create
  one — no credit card), then creates the database, applies the schema,
  sets the encryption key, deploys, and verifies the live URL —
  automatically. You need Node.js installed, nothing else.

The result is a free `https://<your-worker>.workers.dev` address. **You do
not need to buy a domain — a domain purchase is NOT required for anything
in this app.** Cloudflare's free `*.workers.dev` subdomain costs $0.

After the first deploy, open the app → **Settings → System**: it detects
your deployed backend on this machine and offers a one-click
**"Use this backend"** button (no copy-pasting). Then register a NEW
account (backend accounts are separate from local ones) and add your AI
key.

The rest of this document explains what that automation does under the
hood, plus the manual path if you ever need it.
...
## 0. One-command deploy (manual alternative)
