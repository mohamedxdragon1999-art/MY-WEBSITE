#!/bin/bash
# NexusCRM launcher for macOS/Linux — double-click this file.
cd "$(dirname "$0")" || exit 1

if command -v node >/dev/null 2>&1; then
  # ── One-click backend deploy (free, no domain needed).
  #    Skips in ~2 seconds if the backend is already deployed and healthy;
  #    only offers to deploy when it is missing. Node is required for it.
