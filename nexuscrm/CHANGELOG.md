...
**Website builder (part 5 of the overhaul):** `site/widgets.js` — cost estimator, 3-step quote funnel (→ same `site_lead` webhook, own inbox subject), filter chips, mobile action bar; fact-driven selection, injected before the sanitiser, AI told not to duplicate; `widgets` option validated/threaded/persisted; `build.widgets`. WCAG AA across all 20 palettes + 40 themes (`aaText`, hue-preserving `--muted` darkening), `:focus-visible` ring + skip link + `<main id="main">` on every page; honest proof heading. Brief intelligence: thousands separators, priced services, colon lists, WhatsApp attribution, named pricing tiers, starting-price tiers. Prompt budget: provenance stripped from the content spec; oversize plans trimmed least-important-first so RULES + widget vocabulary always reach the model.
Files: `backend/src/index.js` (11,884 lines), `backend/src/site/{widgets.js (new), model_output.js, design_brain.js, catalog.js}`, `backend/src/{nx_site_builder.js, nx_brief.js, nx_copywriter.js}`, `backend/src/validators/brief.js`, `NexusCRM_V4_Hardened.html`, `server.js`, `tests/{test_site_widgets.mjs (new, 83), test_model_output.mjs (new, 84), test_safe_html.mjs, test_module_architecture.mjs, run_all.mjs}`.
Tests: battery 83 suites green (see `AI_HARDENING.md` Batch 10 for the per-area proof), 65/65 routes.
Risk: medium — generated pages gain markup/CSS/JS (widgets) and an extra `window.NX_LEAD_URL`; all additive, sanitiser-verified, jsdom-executed.

---

## 2026-09-05 — SECURITY + NIM UNIT (v0.0.0.0.19, post-release hardening): tenant isolation, swallow sweep, origin policy, "any NIM model works"

**Security (all reproduced, fixed, re-proved by `audit/repro/*`):** S2 snapshot IDOR (GET/DELETE/restore ownership-scoped, foreign → 404); N7 forced-publish bypass (publish state changes only via `publish_action` or a publish-only body — settings/regenerate/rename can never flip a draft live; FE mirrors it); N8 lead endpoint survives every save (`resolveLeadUrl`: valid explicit → remembered → workspace endpoint; a REJECTED explicit URL disconnects the form + `input_warnings`); B12 unscoped tenant SQL (new `db/tenant.js` — `Tenant(env, ws)` scoped helpers, every literal either binds `workspace_id` or carries a documented `/* tenant-scope: … */` marker; static lint 274/0 in the new multi-tenant suite); B13 submission delete scoped; SSRF (`security/ssrf.js` — literal IPs v4/v6/mapped/decimal/octal, link-local `169.254.169.254`, DNS-rebinding-safe redirects, port allow-list, capped reads); CORS/frame-ancestors reflection on `/api/public/site/:slug` (`ALLOWED_ORIGINS` grant policy: exact origins + wildcard subdomains, `null`/`*`/junk never granted, `/s/:slug` always SAMEORIGIN; documented in `wrangler.toml` + `DEPLOY.md §9`).
**Silent-catch sweep:** `observability/log.js` (`nxLog`, `logSwallow`, `logFailure`, `logged`, `redact`) — every `catch` in `backend/src/` now logs with a scope or documents why the verdict is the handling (lint G in `test_tenant_isolation`); provider failures + breaker trips logged key-free (`ai.provider.failure` / `ai.provider.breaker_open`).
**Strict brief validation:** `validators/brief.js` (`validateSiteBody` — every builder field typed/capped/whitelisted incl. the visual-editor `overrides`; 400 names the field; 413 on oversize).
**NVIDIA NIM — any model in the catalog actually works:** new `providers/nim.js` + `providers/errors.js`. Base-URL-aware catalog (self-hosted NIM lists what it serves), chat-capable filter (vision-chat kept; embeddings/rerank/ASR/TTS/image/bio/`/v1/vlm`-only hidden) with 🧠/👁/💻 meta; pasted `build.nvidia.com` links / backticks / `models/` → clean ids, comma fallback chains; reasoning models: 1024-token thinking floor, `reasoning_content` + `<think>` split from the answer in JSON **and** SSE (boundary-safe), one automatic 4× budget retry on `finish_reason: length`, health probe never false-fails; **learned per-model request adaptation** from the provider's own 400 body (max_tokens ceiling/context arithmetic, system-role folding, role alternation, `response_format`, `max_completion_tokens`, fixed sampling) — applied and retried in the same request, remembered, never trips the breaker; streaming shares the same brain (+ Retry-After honoured, failures counted); `POST /ai/models/check` verdict API (ok/not_chat/model_not_found+“did you mean” from the live catalog/bad_key/rate_limited/timeout); SSE pumps never stall on empty frames (was up to 15 s late for reasoning models); shared `nimExtractJson` (fences/prose/balanced braces) replaces six greedy ad-hoc parsers; FastAPI `detail[]` errors humanised. **Instructions:** the workspace personality prompt is now MERGED with each task's protocol (it used to replace it — agent JSON protocol, webchat visitor guardrails and live-data context were silently lost); new `NX_ASSISTANT_LAW` operating rules; visitor-facing webchat prompt rewritten. **FE:** NVIDIA "Other model" free-text input + "🔎 Check this model" + NIM endpoint field + custom-provider model field (previously read from non-existent inputs), thinking indicator in all three chat UIs, local-mode mirrors (catalog filter, base URL, check route, `<think>` split).
**F17 browser stream stall (audit CRIT #2, browser reader) FIXED:** `streamProviderDirect()` `pull()` now loops until it has forwarded a frame or closed, `[DONE]` ends the stream, a provider `{error}` frame is relayed, and buffered `<think>` text is flushed on finish. Pinned behaviourally in `test_overhaul` (A2: 9 real SSE shapes × no-stall + thinking-separated; control run against the pre-fix HTML fails 10/10). Repro `stream_real3` 9/9 ✅, `stream_fix_verify` now verifies the shipped reader.
**B7 build-report race FIXED (reproduced 6/12 crossed → 0/12):** `generateSiteHtml(env, ws, opts, rep)` writes the build report / design rationale into a caller-owned `rep` object instead of module globals; under concurrent builds one tenant no longer receives another tenant's industry/summary/brief facts. Pinned in `test_tenant_isolation` §B2 (concurrent creates + regenerates).
**B9 canned webchat reply FIXED:** when the assistant is down (bad key, model, credits, outage) or the daily cap is hit, the visitor is told an automated message was saved and a person will reply; the widget labels the bubble (`{meta:{auto,reason}}` → `data-nxw-auto`), the inbox stores it as `AUTO:` non-AI outbound (`ai_generated=0`, shown as "auto-reply (assistant unavailable)"), the event is logged `webchat.auto_reply` with the provider error class, and visitor memory replays turns by `direction` (not text prefix). Pinned in `test_webchat_widget` (7 checks incl. recovery).
**Instructions:** `buildMessages` MERGES the workspace personality prompt with each task's protocol (previously it silently replaced them — agent JSON protocol, webchat guardrails and live-data context were lost whenever a personality was set); `NX_ASSISTANT_LAW` ground rules for general completions; visitor webchat prompt rewritten (site facts only, never invent prices/hours, visitor's language).
**Docs:** `NVIDIA_NIM.md` (operator guide), `DEPLOY.md §9`, `backend/wrangler.toml` comments.
Files: `backend/src/index.js` (11,855 lines; provider error taxonomy extracted), `backend/src/{providers/nim.js,providers/errors.js,security/ssrf.js,observability/log.js,db/tenant.js,validators/brief.js,middleware/http.js,nx_site_output.js,nx_browser.js}`, `NexusCRM_V4_Hardened.html`, `tests/{test_nim_models.mjs (new, 124),test_tenant_isolation.mjs (new, 64),d1mock.js,run_all.mjs,test_module_architecture.mjs,test_safe_html.mjs}`, `audit/repro/{site_probe,lead_loss_after_regenerate}.mjs`, `NVIDIA_NIM.md`, `DEPLOY.md`, `backend/wrangler.toml`.
Tests: battery 80 suites / 2,476 checks green in 252 s, 100 % route coverage (64/64, incl. `/ai/models/check`); repro scripts `ssrf_probe` 15/15, `idor_sweep`, `site_probe` (origin contract + N7), `webchat_inbox`, `lead_loss_after_regenerate` all ✅.
Risk: medium — provider request shape is now adaptive (bounded: ≤3 adaptations/attempt, learned table capped at 500 models); pure modules are unit-tested and index.js only orchestrates.

---

## 2026-09-05 — RELEASE v0.0.0.0.19 "UNDERSTAND → WRITE → DESIGN": fact-only website builder + honest built-in assistant

Benchmarked the builder against prompt-to-website leaders (Google AI Studio Build, Wix Harmony, Framer Wireframer, Squarespace Blueprint, Durable, Relume, v0, Lovable, Stitch) and closed the gaps: an English brief is now UNDERSTOOD (`nx_brief.js` — 52 industries, services/people/founded/hours/prices/proof/differentiators/audience/location/contact/CTA intent/personality/language, owner questions for what is missing), WRITTEN in that voice from facts only (`nx_copywriter.js` — slot budgets, per-archetype sitemap, evidence-filtered FAQ, `warnings[]` for anything suggested), and DESIGNED with real tokens (`nx_site_builder.js` — contrast-checked palettes, font pairs, per-industry art). Reference-template identity residue scrubbed at source (`nx_identity.js`, leaks = 0). Generated-site assistant rebuilt: dead brain replaced by a fact-only engine (owner FAQ → topic rules → per-service cards → proof facts → honest "I won't guess" + how to reach the owner), default provider is the built-in engine (no ~400 MB model auto-download, honest status line, CPU fallback opt-in), dev panel labels restored, lead card ships only when a lead can be delivered. Copy defects fixed (grammar, `visit` CTA false-positive, builder meta leaking into chat knowledge, "Visit us?" chips).

Files: `backend/src/index.js`, `nx_template.js`, `NexusCRM_V4_Hardened.html`, `backend/src/nx_brief.js`, `backend/src/nx_copywriter.js`, `backend/src/nx_site_builder.js`, `backend/src/nx_identity.js`, `nx_identity.js`, `backend/src/nx_safe_html.js`, `backend/src/nx_site_output.js`, `tests/test_template_assistant.mjs` (52/0, new), `tests/test_safe_html.mjs` (76/0), `tests/run_all.mjs`, `tests/test_aurora.mjs`, `tests/test_template_*.mjs`, `tests/test_backend.mjs`, `tests/test_blueprint.mjs`. Detail: `audit/CHANGES-v0.0.0.0.19.md`, `audit/AUDIT-2026-09-03.md`, `audit/PLAN-v0.0.0.0.19.md`. Version: `v0.0.0.0.19`.
Risk: medium — the template runtime script changed (3 byte-identical copies, JSON round-trip); full battery 78/78 green, 100 % route coverage.
